...
type (
	...
	// ExecuteFunc is called for each executed instruction. The passed reader
	// is only valid until the next call to ExecuteFunc. Returning an error
	// terminates remote program execution. The program must be fully executed
	// for any changes to be committed. If an instruction fails, the program is
	// rolled back.
...
func (s *Session) ExecuteProgram(program Program, input []byte, payment PaymentMethod, execute ExecuteFunc) error {
	...
	if err := payment.Pay(stream, program.Budget); err != nil {

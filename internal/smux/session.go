...
func (tm *tryMutex) TryLock(t time.Duration) bool {
	tm.once.Do(tm.init)
	select {
	case <-tm.lock:
		return true
...
type Session struct {
	...
	sendMu      tryMutex // ensures only one thread sends at a time
...
func (s *Session) OpenStream() (*Stream, error) {
	...
	if _, err := s.writeFrame(newFrame(cmdSYN, sid), time.Now().Add(s.config.WriteTimeout)); err != nil {
		return nil, fmt.Errorf("writeFrame: %w", err)
	}
...
func (s *Session) readFrame(buffer []byte) (f Frame, err error) {
	...
	if _, err := io.ReadFull(s.conn, buffer[:headerSize]); err != nil {
		return f, fmt.Errorf("readFrame: %w", err)
	}
...
func (s *Session) writeFrame(frame Frame, timeout time.Time) (int, error) {
	...
	// to grab the sendMu.
	if !s.sendMu.TryLock(time.Until(timeout)) {
		return 0, errTimeout
	}
